...
package com.notification.notyos10.customviews.swipedown;
...
public class ToolbarPanelLayout extends RelativeLayout {
    ...
    private float mDistanceBottom;
    private float mMatchCurrentY;
    private float mMatchOldY;
    private float mUpX;
    ...
    public ToolbarPanelLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        shadowDrawable = resources.getDrawable(R.drawable.bg_drop_shadow);
    ...
    protected void onFinishInflate() {
        ...
        toolbar = (RelativeLayout) findViewById(toolbarId);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            mMatchCurrentY = ev.getY();
            mMatchOldY = ev.getY();
        } else if (ev.getAction() == MotionEvent.ACTION_MOVE) {
            if (ev.getY() - mMatchOldY > 0) {
                mDistanceTouch = ev.getY() - mMatchCurrentY;
            } else {
                mMatchCurrentY = ev.getY();
                mDistanceTouch = 0;
                mCurrentTime = System.currentTimeMillis();
        ...
        if (ev.getAction() == MotionEvent.ACTION_CANCEL || ev.getAction() == MotionEvent.ACTION_UP) {
            isTouch = false;
            mUpX=ev.getY();
            mCurrentTime = System.currentTimeMillis() - mCurrentTime;
            mDistanceBottom = ev.getY();
    ...
    void dispatchOnDrawerSlide(float slideOffset) {
        if (toolbarPanelListener != null) {
            toolbarPanelListener.onPanelSlide(toolbar, panel, slideOffset, isTouch, mCurrentTime, mDistanceTouch,mUpX);
        }
    }
}
